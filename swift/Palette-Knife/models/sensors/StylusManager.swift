...
final class StylusManager{
   ...
   static private var currentRecordingPackage:StylusRecordingPackage!
    ...
    @objc static func advanceRecording(){
        ...
        let sample = currentLoopingPackage.getRecording(hash:elapsedTime);
        if(sample != nil){
            print("sample found at time",elapsedTime,currentLoopingPackage.lastSample,sample?.stylusEvent);
        ...
        if(elapsedTime>currentLoopingPackage.lastSample){
...
private class StylusRecordingPackage{
    var dx:Recording;
    var dy:Recording;
    var x:Recording;
    var y:Recording;
    ...
    func getRecording(hash:Float)->Sample?{
        if(self.samples.contains(hash)){
            ...
            let sample = Sample(dx: dx.get(id:nil), dy: dy.get(id:nil), x: x.get(id:nil), y: y.get(id:nil), force: force.get(id:nil), angle: angle.get(id:nil), stylusEvent: stylusEvent.get(id:nil))
            return sample;
        }
        return nil;
    }
...
struct Sample{
    ...
    var x:Float?
    var y:Float?
    var stylusEvent:Float?
    var force:Float?
    var angle:Float?
    
    init(dx:Float,dy:Float,x:Float,y:Float,force:Float,angle:Float,stylusEvent:Float){
        self.dx=dx;
        self.dy=dy;
        self.x=x;
        self.y=y;
