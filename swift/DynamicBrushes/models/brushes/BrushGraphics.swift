...
extension Brush {
    ...
    func drawIntoContext(context: BrushGraphicsView, brushInfo:BrushStateStorage,stylusInfo:(Double,Double,Double,Double,Double,Int)){
        if !Debugger.brushGfx && !Debugger.inputGfx && !Debugger.outputGfx { return }
       ...
       // print("## drawing into context for brush ", self.id, " state is ", state  )
        ...
        if active {
            ...
            context.scene!.addBrushGraphic(id:self.id, ox:self.params.ox, oy:self.params.oy, r: self.params.rotation, x: self.params.x, y:self.params.y, cx: self.params.cx, cy:self.params.cy, ix:ix, iy:iy)
        }
